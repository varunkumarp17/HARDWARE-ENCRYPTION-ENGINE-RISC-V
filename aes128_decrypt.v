module aes128_decrypt (
    ...
    // ---------------- RCON ----------------
    initial begin
        Rcon[1]=32'h01000000; Rcon[2]=32'h02000000; Rcon[3]=32'h04000000;
        Rcon[4]=32'h08000000; Rcon[5]=32'h10000000; Rcon[6]=32'h20000000;
        Rcon[7]=32'h40000000; Rcon[8]=32'h80000000; Rcon[9]=32'h1b000000;
    ...
    always @(*) begin
        ...
        for(i=4;i<44;i=i+1)
            w[i]=(i%4==0)?(w[i-4]^subword(rotword(w[i-1]))^Rcon[i/4]):(w[i-4]^w[i-1]);

        for(i=0;i<11;i=i+1)
            round_key[i]={w[4*i],w[4*i+1],w[4*i+2],w[4*i+3]};

        // Initial step
        state = ciphertext ^ round_key[10];

        // Rounds
        for(round=9; round>0; round=round-1) begin
            ...
            // InvSubBytes
            for(i=0;i<16;i=i+1)
                state[127-8*i -:8]=inv_sbox[state[127-8*i -:8]];
            ...
            for(i=0;i<4;i=i+1) begin
                ...
                state[127-32*i -:8]=mul(a0,8'he)^mul(a1,8'hb)^mul(a2,8'hd)^mul(a3,8'h9);
                state[119-32*i -:8]=mul(a0,8'h9)^mul(a1,8'he)^mul(a2,8'hb)^mul(a3,8'hd);
                state[111-32*i -:8]=mul(a0,8'hd)^mul(a1,8'h9)^mul(a2,8'he)^mul(a3,8'hb);
                state[103-32*i -:8]=mul(a0,8'hb)^mul(a1,8'hd)^mul(a2,8'h9)^mul(a3,8'he);
            end
        end
